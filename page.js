...
// Palette vivace dedicata ai due pesci mascotte (indipendente dai colori
// dell'interfaccia, così il resto del tema resta invariato).
const FISH_COLORS = {
  didi: { light: "#A6FFF0", main: "#1FE0C2", deep: "#0C8B75" },
  gogo: { light: "#FFD9A0", main: "#FF9A33", deep: "#DE6A0A" },
};

function FishSVG({ colors, uid }) {
  const bodyGradId = `aqFishBody-${uid}`;
  const bellyGradId = `aqFishBelly-${uid}`;
  return (
    <svg viewBox="0 0 100 64" width="62" height="40">
      <defs>
        {/* gradiente principale: dà al corpo un effetto tondo e "gommoso" da giocattolo */}
        <radialGradient id={bodyGradId} cx="36%" cy="32%" r="78%">
          <stop offset="0%" stopColor={colors.light} />
          <stop offset="55%" stopColor={colors.main} />
          <stop offset="100%" stopColor={colors.deep} />
        </radialGradient>
        {/* pancino più chiaro, sfumato verso il basso */}
      ...
      {/* coda a paletta arrotondata, più giocosa della forcuta originale */}
      <path
        d="M78,32 C90,13 105,16 96,32 C105,48 90,51 78,32 Z"
        fill={colors.deep}
        opacity="0.92"
      />

      {/* pinna dorsale piccola e arrotondata */}
      <path
        d="M36,15 C40,3 55,2 60,11 C52,15 44,16 36,15 Z"
        fill={colors.deep}
        opacity="0.75"
      />

      {/* pinna pettorale a goccia */}
      <ellipse
        cx="47"
        cy="45"
        rx="10.5"
      ...
      {/* corpo: bolla tonda e paffuta, stile mascotte */}
      <ellipse
        cx="46"
        cy="32"
        rx="34"
      ...
      {/* pancino più chiaro per un effetto giocattolo/cartone */}
      <ellipse cx="44" cy="45" rx="23" ry="12" fill={`url(#${bellyGradId})`} />

      {/* riflesso lucido in alto, per un aspetto brillante e amichevole */}
      <ellipse
        cx="54"
        cy="11"
        rx="13"
        ...
        fill="#FFFFFF"
        opacity="0.22"
        transform="rotate(-14 54 11)"
      />

      {/* occhio grande ed espressivo, il tratto distintivo della mascotte */}
      <circle cx="29" cy="27" r="11.5" fill="#FFFFFF" />
      <circle cx="30.5" cy="28" r="7.4" fill={COLORS.deep} />
      <circle cx="27.5" cy="24.5" r="2.7" fill="#FFFFFF" />
      <circle cx="33.5" cy="31" r="1.2" fill="#FFFFFF" opacity="0.85" />
    </svg>
  );
}

function SwimmingFish({ name, colors, swimDuration, swimDelay, topOffset, pitch, playBlub }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Il pesce è disegnato con la testa a sinistra: lo specchiamo qui,
            su un elemento separato dal bottone, così la stessa animazione
            di "sussulto" al click (che anima il bottone) non lo fa tornare
            per un istante nel verso sbagliato. Ora la testa punta nella
            direzione in cui l'animazione lo fa nuotare (verso destra). */}
        <span style={{ display: "inline-block", transform: "scaleX(-1)" }}>
          <FishSVG colors={colors} uid={name} />
        </span>
      </button>
      {reacting && (
        <div className="aq-fish-caption" style={{ borderColor: colors.main }}>
          <span style={{ color: colors.main, fontWeight: 700 }}>{name}</span>
...
export default function AcquaCalc() {
  ...
  return (
    ...
    >
      ...
      <SwimmingFish name="Didi" colors={FISH_COLORS.didi} swimDuration={28} swimDelay={0} topOffset="16%" pitch={0.7} playBlub={playBlub} />
      <SwimmingFish name="Gogo" colors={FISH_COLORS.gogo} swimDuration={34} swimDelay={-9} topOffset="27%" pitch={0.3} playBlub={playBlub} />
